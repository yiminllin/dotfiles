...
- When routing Phoenix/HIL/ZML or multi-topic log work, seed and preserve the relevant `Topic Ledger`; include exact field/topic names, source artifact, time window or attempt, and extraction/compare command before comparing behavior or declaring mismatch. Route read-only inspection, evidence, ZML/log audit, and pass/fail comparison to `phoenix_inspector` as described below.
- When a Phoenix/ZML investigation repeats, preserve the exact inventory/fields/extract/summary/compare command recipe, outputs, evidence limits, and proves/does-not-prove boundaries in the handoff or final answer instead of hand-rolling another one-off script.
- When delegating agent Phoenix Inspector report workflows, prefer `--format both --out-dir /tmp/pi/<short-task>` and keep extract CSVs under the same task directory; do not require `--out-dir` for human/manual stdout use.
- Phoenix Inspector command choice: unknown source/artifacts -> `inventory`; known signal/field but unknown topic -> `fields`/`find-field`; known topic + field -> `extract`; known topic/all values -> `extract --all-fields`; CSV summary -> `summary`; known pass/fail topic/field/preset -> `compare`; recent HIL source lookup -> `recent-hil`.
...
1. For read-only Phoenix/HIL/GHA/ZML/log inspection—GHA HIL failures, exact run/job or S3 inventory, recent HIL source or passing-run discovery, one-run evidence collection, local Phoenix log inspection, ZML topic/extract/CSV work, pass/fail or before/after signal comparison, or requests like "what topic/log did you read?"—load `phoenix_inspector`, not `phoenix-workflows`.
2. For Phoenix SIL/no_sync/local scenario handling, HIL workflow launches/execution, reruns, fetch/upload workflows, or any action that can start/modify runtime state, load `phoenix-workflows` before delegating. PR babysit owns PR status/comments/CI monitoring, but PR CI retry/rerun/relaunch requests involving failed HIL jobs, HIL workflow dispatch, `p2-zip-system-hil-build.yml`, `gh workflow run`, or hardware-like launch semantics must use `phoenix-workflows` before dispatch. Phoenix/SIL artifact upload, presign, Baraza, LogPlots, or dashboard upload requests also go through `phoenix-workflows` fetch/upload decision mode before `$upload_local_log_to_s3` or repo-script leaf helpers. If logs are already collected and the user asks to inspect them, hand off to `phoenix_inspector`.
