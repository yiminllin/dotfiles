...
def _ensure_gh_authenticated() -> None:
    try:
        ...
        print("GitHub CLI authentication is unavailable; authenticate outside this helper", file=sys.stderr)
        raise RuntimeError(
            "gh auth status failed; this helper will not start or repair authentication"
        ) from None
...
def _maybe_emit_permissions_hint(error: str, *, context: str) -> None:
    ...
    if "forbidden" in lowered or "permission" in lowered:
        print(
            f"{context}: GitHub permissions are insufficient; repair authorization outside this helper.",
            file=sys.stderr,
        )
