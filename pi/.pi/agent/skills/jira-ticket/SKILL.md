...
Jira reads require network/auth approval. Creating, editing, moving, linking, or
commenting also requires explicit approval for the exact write. Never initiate
login, refresh credentials, install the CLI, or expose credential material; if
`jira` is missing or authentication fails, report the blocker and stop.
...
5. Confirm before crossing the network/auth boundary and again before writing to Jira, including assignee, parent, due-date handling, and any parent/child/epic writes.
6. Create the ticket:
...
- If Jira auth fails, stop and ask the user to refresh Jira CLI credentials or environment variables themselves; never start an auth flow.
